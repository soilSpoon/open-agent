...
export class PromptTemplateEngine {
  ...
  generateMainPrompt(v: TemplateVariables): string {
    ...
    parts.push("");
    parts.push(
      "You are an autonomous implementation agent working on the OpenSpec change.",
    );
    parts.push("");
    ...
    if (v.hasCodebasePatterns) {
      ...
      parts.push("");
      parts.push(
        "These patterns discovered in previous iterations should guide your implementation:",
      );
      parts.push("");
    ...
    parts.push("");
    parts.push(
      "1. **Explore & Plan**: Read the current codebase. Identify existing patterns.",
    );
    parts.push(
      "2. **Implement**: Make the requested changes following existing conventions.",
    );
    ...
    parts.push(
      '  "codebasePatterns": ["Pattern discovered in this iteration"],',
    );
    parts.push(
      '  "gotchas": ["Any issues encountered and how they were resolved"],',
    );
    parts.push(
      '  "summary": "Clear synthesis of what was done and key insights",',
    );
    ...
    parts.push("");
    parts.push(
      'If validation fails, set "complete": false and include failure details.',
    );
  ...
  generateAnalysisPrompt(
    ...
    parts.push("");
    parts.push(
      "1. **Root Cause**: What technically caused this failure? Be specific.",
    );
    parts.push(
      "2. **Fix Plan**: What exactly needs to change in the next attempt?",
    );
    parts.push(
      "3. **Key Insight**: What pattern or lesson should be remembered?",
    );
    parts.push("");
  ...
  private formatRecentProgress(logs: IterationLog[]): string {
    ...
    for (const log of recent) {
      ...
      lines.push(
        `${icon} Iteration ${log.iteration}: ${log.summary || "No summary"}`,
      );
